...
public class FindAndReplaceInString {
    public static String findReplaceString(String s, int[] indices, String[] sources, String[] targets) {
        TreeMap<Integer,Integer> sortedIndxMap = new TreeMap<>(Collections.reverseOrder());
        int i=0;
        StringBuilder sb = new StringBuilder(s);
        for(int indx: indices){
            sortedIndxMap.put(indx,i++);
        }
        for(Map.Entry<Integer,Integer> index:sortedIndxMap.entrySet()){
            int value = index.getValue();
            int key = index.getKey();
            String source = sources[value];
            String target = targets[value];
            if(s.substring(key,key+source.length()).equalsIgnoreCase(source)){
                sb.replace(key,key+source.length(),target);
            }
        }
        return sb.toString();
    }
