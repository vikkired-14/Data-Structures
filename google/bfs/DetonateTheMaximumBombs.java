...
public class DetonateTheMaximumBombs {
    ...
    public static int maximumDetonation(int[][] bombs) {
        ...
        for(int i=0;i< bombs.length;i++){
            for(int j=0;j< bombs.length;j++){
                if(i!=j){
                    ...
                    int x2 = bombs[j][0];
                    int y2 = bombs[j][1];
                    if((long)r1*r1>= (long)(x2-x1)*(x2-x1) + (long)(y2-y1)*(y2-y1)){
                        bombMap.computeIfAbsent(i,k->new ArrayList<>()).add(j);
                    }
                }
            }
        }
        int count=0;
        for(int i=0;i< bombs.length;i++){
            count = Math.max(count,bfs(bombMap,i));
        }
        return count;
    }

    private static int bfs( Map<Integer, List<Integer>> bombMap, int i) {
        ...
        while (!queue.isEmpty()){
            int idx = queue.poll();
            if(bombMap.containsKey(idx)){
                for(int index:bombMap.get(idx)){
