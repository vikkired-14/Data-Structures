...
public class FindAllPossibleRecipesFromGivenSupplies{
    ...
    public List<String> findAllRecipes(String[] recipes, List<List<String>> ingredients, String[] supplies) {
        ...
        Map<String,List<String>>ingredientsReceipeMap = new HashMap<>();
        Queue<String> queue = new LinkedList<>();
        for(int i =0;i<recipes.length;i++){
            String recipe = recipes[i];
            for(String ingredient: ingredients.get(i)){
                ingredientsReceipeMap.computeIfAbsent(ingredient,s->new ArrayList<>()).add(recipe);
        ...
        for(String receipe : recipes){
            if(!indegree.containsKey(receipe)){
                queue.offer(receipe);
                ans.add(receipe);
            }
        }
        while(!queue.isEmpty()){
            String receipe = queue.poll();
            if(ingredientsReceipeMap.containsKey(receipe)){
                for(String ingredient : ingredientsReceipeMap.get(receipe)){
